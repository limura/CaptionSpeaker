...
async function GetYoutbeiV1PlayerData() {
  ...
  const ytCfgJSON = document
    ...
    .snapshotItem(0)
    .innerText.replace(/^[\s\S]*ytcfg\.set\({/g, "{")
  ...
  try {
    ...
    if (!resData?.captions?.playerCaptionsTracklistRenderer?.captionTracks) {
      ...
      const ytInitialPlayerResponseElement = document
        .evaluate(
      ...
      const ytInitialPlayerResponseCode =
        ytInitialPlayerResponseElement.innerText
          .replace("var ytInitialPlayerResponse = ", "")
          .replace(/};var .*/, "}");
      const ytInitialPlayerResponseObject = JSON.parse(
        ytInitialPlayerResponseCode
      );
...
async function GetCaptionDataUrl() {
  ...
  let captionTracks =
    player_response_obj?.captions?.playerCaptionsTracklistRenderer
      ?.captionTracks;
  let playLocaleCaptionBaseUrl = captionTracks?.filter(
    (obj) => obj?.languageCode == playLocale
  )[0]?.baseUrl;
...
async function FetchCaptionData(isForceFetch = false) {
  try {
    if (isCaptionDataFetching) {
      return undefined;
    }
    isCaptionDataFetching = true;
    console.log("FetchCaptionData start");
    const videoId = GetVideoId();
    if (videoId == CURRENT_VIDEO_ID && !isForceFetch) {
    ...
    const storageResult = await getStorageSync([
      "isDisableSpeechIfSameLocaleVideo",
    ]);
    if (
      guessedOriginalCaptionLanguage == playLocale &&
      storageResult.isDisableSpeechIfSameLocaleVideo
    ) {
      captionData = {};
      isCaptionDataFetching = false;
      return undefined;
...
function CaptionDataToTimeDict(captionData) {
  ...
  if (!events) {
    console.log("CaptionDataToTimeDict(): error. events not found");
    return;
  }
  let captionArray = events
    .map((obj) => {
      ...
      return {
        tStartMs: tStartMs,
        segment: segment,
        time: FormatTimeFromMillisecond(tStartMs),
      };
    })
    .filter((obj) => {
...
function GetYtpVolumePanelValue(fallbackVolume) {
  const ytpVolumePanelCurrentValue = document
    .evaluate(
      "//span[@class='ytp-volume-area']/div[@aria-valuenow]/@aria-valuenow",
      document,
      null,
      XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,
      null
    )
    .snapshotItem(0)?.value;
...
function volumeOverride(
  videoElement,
  originalVolume,
  targetVolumeMagnification
) {
  overrideVolumeCount += 1;
  videoElement.volume = Math.min(
    Math.max(0, originalVolume * targetVolumeMagnification),
    1
  );
}
...
function AddSpeechQueue(text, storageResult, videoElement) {
  ...
  if (
    storageResult.isOverrideOriginalVolumeEnabled &&
    originalVolume &&
    videoElement
  ) {
    ...
    volumeOverride(
      videoElement,
      targetVolume,
      storageResult.overrideOriginalVolumeMagnification
    );
    utt.onend = (event) => {
      const targetVolume = GetYtpVolumePanelValue(originalVolume);
...
function CheckAndSpeech(currentTimeText, storageResult, videoElement) {
  if (!currentTimeText) {
    console.log("currentTimeText is nil");
    return;
  }
  if (currentTimeText == prevSpeakTime) {
    return;
  }
  if (
    storageResult.isDisableSpeechIfChaptionDisabled &&
    !CheckCaptionIsDisplaying()
  ) {
    return;
  }
...
async function CheckVideoCurrentTime(loadGapSecond = 0.0) {
  ...
  const storageResult = await getStorageSync([
    ...
    "volume",
    "isStopIfNewSpeech",
    "isDisableSpeechEmbeddedSite",
    "isOverrideOriginalVolumeEnabled",
    "overrideOriginalVolumeMagnification",
  ]);
  if (!IsTargetUrlWithOption(location.href, storageResult)) {
    return;
  }
  const isEnabled =
    storageResult?.isEnabled || typeof storageResult?.isEnabled == "undefined";
  if (!isEnabled) {
    return;
  }
  let videoElement = document
    .evaluate(
      "//video[contains(@class,'html5-main-video')]",
      document,
      null,
      XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,
      null
  ...
  if (!videoElement) {
    console.log(
      "CheckVideoCurrentTime videoElement is not found:",
      videoElement
    );
    return;
  }
  ...
  if (!IsValidVideoDuration(duration, captionData)) {
    console.log(
      "CheckVideoCurrentTime is not valid VideoDuration. currentTime:",
      currentTime,
      "duration:",
      duration,
      "captionData:",
  ...
  for (let gap = loadGapSecond; gap >= 0; gap -= 1) {
    ...
    if (prevCheckVideoTimeText == timeText) {
      return;
    }
...
function StartVideoTimeChecker() {
  if (CURRENT_VIDEO_ID == GetVideoId()) {
    CheckVideoCurrentTime().catch((e) => {
      console.log(
        "CheckVideoCurrentTime got error. video time checker stop:",
        e
      );
  ...
  for (const [key, { oldValue, newValue }] of Object.entries(changes)) {
    switch (key) {
      ...
      case "isDisableSpeechEmbeddedSite":
        if (
          location.href.indexOf("https://www.youtube.com/embed/") == 0 &&
          !newValue
        ) {
          StartVideoTimeChecker();
        }
